...
class OnlineGamePdfControllerTest {

    private IOnlineGamePdfService pdfService;
    private OnlineGamePdfController controller;

    @BeforeEach
    void setup() {
        pdfService = Mockito.mock(IOnlineGamePdfService.class);
        controller = new OnlineGamePdfController(pdfService);
    }

    @Test
    void deveExportarPdfComSucesso() {
        byte[] pdfBytes = "fake-online-pdf".getBytes();
        ResponseEntity<ByteArrayResource> responseMock = ResponseEntity.ok()
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=online-games.pdf")
        ...
        assertTrue(response.getHeaders().getFirst(HttpHeaders.CONTENT_DISPOSITION).contains("online-games.pdf"));
