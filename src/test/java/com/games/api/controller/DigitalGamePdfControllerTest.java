...
class DigitalGamePdfControllerTest {

    private IDigitalGamePdfService pdfService;
    private DigitalGamePdfController controller;

    @BeforeEach
    void setup() {
        pdfService = Mockito.mock(IDigitalGamePdfService.class);
        controller = new DigitalGamePdfController(pdfService);
    }

    @Test
    void deveExportarPdfComSucesso() {
        byte[] pdfBytes = "fake-digital-pdf".getBytes();
        ...
        assertTrue(response.getHeaders().getFirst(HttpHeaders.CONTENT_DISPOSITION).contains("digital-games.pdf"));
